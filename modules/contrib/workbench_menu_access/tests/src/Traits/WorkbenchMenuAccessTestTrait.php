...
trait WorkbenchMenuAccessTestTrait {
   ...
   *   A vocabulary entity.
   */
  public function createTerms(Vocabulary $vocabulary) {
    for ($i = 1; $i <= 10; $i++) {
      if ($i <= 5) {
        $term = Term::create([
          'vid' => $vocabulary->id(),
   ...
   */
  public function addUserToSection(AccessSchemeInterface $scheme, AccountInterface$account, array $section_ids) {
   ...
   */
  public function addMenu($menu_name = 'main') {
    $menu = Menu::create([
      'id' => $menu_name,
      'label' => $menu_name,
